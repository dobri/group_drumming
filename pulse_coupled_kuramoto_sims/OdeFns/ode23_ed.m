function varargout = ode23_ed(ode,tspan,y0,options,varargin)
...
%         ieout = [ieout, ie];
        % Ed Large added this 2/1/2020
        for ixt = 1:length(te) % length(te) can be > 1
            teLast = teout(find(ieout == ie(ixt)));
            if ~isempty(teLast)
                teLast = teLast(end);
            else
                teLast = -100;
            ...
            if te-teLast > .250
                idx = find(abs(stim.t - te(ixt)) == min(abs(stim.t - te(ixt)))); idx = idx(1);
                Nmin = min(idx+pulse.N-1, stim.N);
%                 stim.x(ie(ixt),idx:Nmin) = stim.x(ie(ixt),idx:Nmin) + pulse.x(1:length(idx:Nmin));
                stim.x(ie(ixt),idx:Nmin) = pulse.x(1:length(idx:Nmin));
